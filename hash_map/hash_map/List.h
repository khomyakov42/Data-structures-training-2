...
template<class T> class list{
	...
	private:
		class Node{
				friend class list<T>;
				Node(Node *_node1, Node *_node2, T value);
				Node *prev, *next;
				T value;
		};
...
template<class T>  list<T>::Node::Node(Node *_node1, Node *_node2, T _value){
	value = _value;
	prev = _node1;
	next = _node2;
}
...
template<class T> void list<T>::erase(iterator _itr){
	assert(_itr.status == list<T>::iterator::NORMAL && l_size != 0);
